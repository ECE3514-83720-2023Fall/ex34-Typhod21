...
BinaryTree<TreeItemType, FunctionType> BinaryTree<TreeItemType, FunctionType>::makeBinaryTree(const TreeItemType *array, int low, int high) {
    ...
    BinaryTree leftSubtree = makeBinaryTree(array, low, mid - 1); //make left sub tree
    BinaryTree rightSubtree = makeBinaryTree(array, mid + 1, high); //make right sub tree

    BinaryTree newTree(array[mid]); //make tree on middle node
